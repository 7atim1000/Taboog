...
import { FaCalendarAlt, FaUserTag, FaTruck, FaReceipt } from 'react-icons/fa';
...
const SupplierInfo = () => {
    ...
    return (
        <div className='bg-gradient-to-r from-blue-500 to-blue-700 rounded-lg shadow-md p-3 h-auto'>
            {/* Compact Header */}
            <div className='flex items-center justify-between mb-2'>
                <div className='flex items-center gap-2'>
                    <FaTruck className='text-white w-4 h-4' />
                    <h3 className='text-sm font-semibold text-white'>معلومات المورد</h3>
                </div>
                
                <div className='flex items-center gap-1 bg-white/20 px-2 py-1 rounded'>
                    <FaReceipt className='text-white w-3 h-3' />
                    <span className='text-xs font-medium text-white'>
                        #{supplierData.buyId || 'NA'}
                    </span>
                </div>
            </div>
            ...
            <div className='space-y-2'>
                    ...
                    <div className='flex items-center gap-2'>
                        <div className='w-8 h-8 bg-white rounded-full flex items-center justify-center'>
                            <span className='text-emerald-700 font-bold text-sm'>
                                {getAvatarName(supplierData.supplierName || 'م')}
                            </span>
                        </div>
                        <div>
                            <h4 className='text-xs text-emerald-100 mb-0.5'>المورد</h4>
                            <h3 className='text-sm font-bold text-white truncate max-w-[120px]'>
                                {supplierData.supplierName || 'الرجاء اختيار المورد'}
                            </h3>
                        </div>
                    </div>
                    
                    {supplierData.balance !== undefined && (
                        <div className='text-right'>
                            <span className='text-xs text-emerald-100 block'>الرصيد</span>
                            <span className={`text-sm font-bold ${
                                Number(supplierData.balance) >= 0 
                                    ? 'text-green-300' 
                                    : 'text-red-300'
                            }`}>
                ...
                <div className='flex items-center justify-between pt-2 border-t border-white/20'>
                    <div className='flex items-center gap-2'>
                        <FaCalendarAlt className='text-emerald-200 w-3 h-3' />
                        <div>
                            <p className='text-xs text-emerald-100'>{formatDate(dateTime)}</p>
                    ...
                    <div className='flex items-center gap-2'>
                        ...
                        <div className='text-right'>
                            <p className='text-xs text-emerald-100'>بواسطة</p>
