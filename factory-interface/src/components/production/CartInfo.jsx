...
import { FaIndustry, FaBox, FaBoxOpen, FaWeightHanging } from "react-icons/fa";
...
const CartInfo = () => {
    ...
    // Calculate totals
    const totalItems = sortedProduceData.length;
    const totalQuantity = sortedProduceData.reduce((sum, item) => sum + item.quantity, 0);
    const totalValue = sortedProduceData.reduce((sum, item) => sum + item.price, 0);
            ...
            <div className='bg-gradient-to-r from-blue-600 to-blue-700 p-4'>
                <div className='flex items-center justify-between'>
                    <div className='flex items-center gap-3'>
                        ...
                        <div>
                            <h2 className='text-white font-bold text-lg'>سلة الإنتاج</h2>
                            <p className='text-blue-100 text-xs mt-1'>تفاصيل المنتجات المنتجة</p>
                        </div>
                    </div>
            ...
            {sortedProduceData.length > 0 && (
                <div className='grid grid-cols-3 gap-2 p-3 border-b border-blue-50'>
                    ...
                    <div className='bg-blue-50 rounded-lg p-2 text-center'>
                        ...
                        <p className='text-lg font-bold text-blue-800'>{totalValue.toFixed(2)}</p>
                    </div>
                </div>
            )}
            ...
            <div className='p-3'>
                {sortedProduceData.length === 0 ? (
                    <div className='flex flex-col items-center justify-center py-12 text-center'>
                        ...
                        <h3 className='text-gray-600 font-medium mb-2'>سلة الإنتاج فارغة</h3>
                        <p className='text-gray-400 text-sm'>الرجاء اختيار منتج لإضافته إلى قائمة الإنتاج</p>
                    </div>
                ) : (
                    <div 
                        ...
                        ref={scrolLRef}
                    >
                        {sortedProduceData.map((item, index) => (
                            <div 
                                key={`${item.id}-${index}`}
                    ...
                    <div className='mt-4 pt-3 border-t border-blue-100'>
                        <div className='flex justify-between items-center'>
                            <div>
                                ...
                                <p className='text-xs text-gray-400'>{totalItems} منتج، {totalQuantity} وحدة</p>
                            </div>
                            <div className='text-right'>
                                <p className='text-xl font-bold text-emerald-700'>{totalValue.toFixed(2)} ر.ع</p>
                        ...
                        <div className='mt-2 pt-2 border-t border-blue-50'>
                            <div className='flex justify-between items-center'>
                                <span className='text-xs text-gray-500'>متوسط قيمة الوحدة:</span>
                                <span className='text-sm font-medium text-blue-700'>
                                    {averagePrice.toFixed(2)} ر.ع
                                </span>
                            </div>
                        </div>
                    </div>
                ...
                <div className='bg-blue-50 border-t border-blue-100 p-3'>
                    <div className='flex items-start gap-2'>
                        ...
                        <p className='text-xs text-blue-600'>
                            يمكنك النقر على أيقونة الحذف لإزالة منتج من قائمة الإنتاج
                        </p>
                    </div>
                </div>
            )}
