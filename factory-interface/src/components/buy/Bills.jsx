...
import { FaCashRegister, FaCreditCard, FaCheckCircle, FaTimesCircle, FaPrint, FaWallet } from 'react-icons/fa';
...
const Bills = ({fetchItems}) => {
    ...
    const buyMutation = useMutation({ 
        ...
        onSuccess: (resData) => {
            ...
            fetchItems({ category: 'all', page: 1 });
            setPayedAmount(0);
        },
        onError: (error) => {
            console.log(error);
    ...
    return (
        <div className='bg-white rounded-xl shadow-lg border border-blue-100 p-4 h-full'>
            ...
            <div className='bg-gradient-to-r from-blue-600 to-blue-700 rounded-lg p-3 mb-4'>
                <h3 className='text-white font-bold text-lg text-center'>فاتورة الشراء</h3>
            </div>
            ...
            <div className='mb-4'>
                <div className='flex justify-between items-center mb-3'>
                    <h4 className='text-sm font-semibold text-gray-700'>طريقة الدفع</h4>
            ...
            {supplierData.supplierName && (
                <div className='mb-4 p-3 bg-gradient-to-r from-emerald-50 to-green-50 rounded-lg border border-emerald-100'>
                    <div className='flex items-center gap-2 mb-2'>
                        ...
                        <span className='text-xs font-medium text-emerald-600'>المورد الحالي</span>
                    </div>
                    <div className='flex justify-between items-center'>
                        ...
                        <span className={`text-sm font-semibold ${Number(supplierData.balance) >= 0 ? 'text-green-600' : 'text-red-600'}`}>
                            {Number(supplierData.balance || 0).toFixed(2)} ر.ع
                        </span>
                    </div>
                </div>
            )}
            ...
            <div className='mt-4 pt-3 border-t border-blue-100 text-center'>
                <p className='text-xs text-gray-500'>
                    {buyData.length === 0 
                        ? 'قم بإضافة منتجات إلى سلة المشتريات' 
                        : `${buyData.length} منتج في السلة`}
                </p>
            </div>
